...
export function AnalysisView({
  ...
  const checklist = [
    ...
    { label: "Insider Buy Conviction (No Net Selling)", passed: (reportData.insiderSentiment?.netShares ?? 0) >= 0, value: reportData.insiderSentiment?.sentiment || "Neutral" }
  ];
...
`;
  ...
  return (
    <div className="space-y-8 w-full max-w-6xl mx-auto px-6 py-10 animate-fade-in relative z-10 print:p-0 print:space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3.5">
          {[
            ...
            { 
              ...
              task: "Metric Sheets Pull", 
              status: (reportData.routedNodes || ["financials"]).includes("financials") ? "Success" : "Pruned", 
              duration: (reportData.routedNodes || ["financials"]).includes("financials") ? "0.8s" : "0.0s" 
            },
            { 
              name: "News Agent", 
              task: "Headline Sentiment", 
              status: (reportData.routedNodes || ["news"]).includes("news") ? "Success" : "Pruned", 
              duration: (reportData.routedNodes || ["news"]).includes("news") ? "2.1s" : "0.0s" 
            },
            { 
              name: "Risk Agent", 
              task: "SEC Item 1A Disclosures", 
              status: (reportData.routedNodes || ["risk"]).includes("risk") ? "Success" : "Pruned", 
              duration: (reportData.routedNodes || ["risk"]).includes("risk") ? "1.5s" : "0.0s" 
            },
            { 
              name: "Insider Agent", 
              task: "Insider Math & Flow", 
              status: (reportData.routedNodes || ["insider"]).includes("insider") ? "Success" : "Pruned", 
              duration: (reportData.routedNodes || ["insider"]).includes("insider") ? "1.3s" : "0.0s" 
            },
      ...
      {/* News, Insider Sentiment, and SEC Risks Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Insider Trading Sentiment Card */}
        <div className="glass-panel p-6 space-y-4 relative overflow-hidden">
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2">
            <Users className="w-4 h-4 text-blue-400" /> Insider Sentiment Engine
          </h3>
          <div className="p-5 rounded-xl bg-slate-900/60 border border-white/5 space-y-3.5">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">Sentiment Index</span>
              <span className={`text-xs font-black px-2.5 py-0.5 rounded-full border ${
                (reportData.insiderSentiment?.sentiment || "Neutral").includes("Bullish")
                  ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                  : (reportData.insiderSentiment?.sentiment || "Neutral").includes("Bearish")
                    ? "bg-rose-500/10 text-rose-400 border-rose-500/20"
                    : "bg-slate-500/10 text-slate-400 border-slate-500/20"
              }`}>
                {reportData.insiderSentiment?.sentiment || "Neutral"}
              </span>
            </div>

            <div className="grid grid-cols-2 gap-2 text-[10px] py-1 border-y border-white/5">
              <div>
                <p className="text-slate-500">Buy/Sell Ratio</p>
                <p className="text-slate-200 font-extrabold mt-0.5">
                  {reportData.insiderSentiment?.buyToSellRatio || "1.00"}
                </p>
              </div>
              <div>
                <p className="text-slate-500">Net Volume</p>
                <p className={`font-extrabold mt-0.5 ${
                  (reportData.insiderSentiment?.netShares || 0) > 0 ? "text-emerald-400" : "text-rose-400"
                }`}>
                  {(reportData.insiderSentiment?.netShares || 0) > 0 ? "+" : ""}
                  {(reportData.insiderSentiment?.netShares || 0).toLocaleString()}
                </p>
              </div>
            </div>

            {reportData.insiderSentiment?.allTimeHighBuy && (
              <div className="p-2 rounded bg-amber-500/10 border border-amber-500/25 flex items-center justify-center gap-1.5 animate-pulse text-[9px] font-black tracking-wider text-amber-400">
                🔥 PEAK CONVICTION ATH BUYING
              </div>
            )}

            <div className="space-y-1.5 pt-1">
              <p className="text-[9px] font-black uppercase text-slate-500 tracking-wider">Recent Transactions</p>
              {reportData.insiderSentiment?.rawTransactions && reportData.insiderSentiment.rawTransactions.length > 0 ? (
                reportData.insiderSentiment.rawTransactions.map((tx: any, idx: number) => (
                  <div key={idx} className="flex justify-between items-center text-[10px] text-slate-400 leading-snug">
                    <span className="truncate max-w-[100px] font-medium">{tx.name}</span>
                    <span className={`font-semibold shrink-0 ${tx.change > 0 ? "text-emerald-400" : "text-rose-400"}`}>
                      {tx.change > 0 ? "+" : ""}
                      {tx.change.toLocaleString()} ({tx.filingDate})
                    </span>
                  </div>
                ))
              ) : (
                <p className="text-[9px] text-slate-500 italic">No large-scale insider filing updates.</p>
              )}
            </div>
          </div>
        </div>
